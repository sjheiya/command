...
class MainWindow(QWidget, myjr.Ui_Form):
    ...
    def showtable(self, data, i, j, offi=0, offj=0):
        if (data.Fields[i] == "rt_susp_flag".upper() and str(int(data.Data[i][j]))[-1] != '0' and self.model.item(
                    j + offj, i + offi)
        ...
        if (self.model.item(j + offj, i + offi)):
            ...
            if (data.Fields[i] == "rt_susp_flag".upper()):
                self.model.item(j + offj, i + offi).setText(
                    "%s%d" % (self.rt_susp_flag[str(int(data.Data[i][j]))[-1]], data.Data[i][j]))
            elif (self.model.horizontalHeaderItem(i).text() == "rt_trade_status"):
                self.model.item(j + offj, i + offi).setText(
                    "%s%d" % (self.rt_trade_status[str(int(data.Data[i][j]))], data.Data[i][j]))
        else:
    ...
    def refreshdataOnce(self):
        data = self.mwsq(self.codes, self.fields)
        if (data.ErrorCode != 0):
            ...
            if (self.mutex.acquire(5)):
                ...
                for var in data.Codes:
                    ...
                    if (self.cursor.fetchall().__len__() == 0):
                        ...
                        self.cursor.execute(sql)
                    try:
                        mindex = data.Fields.index("rt_susp_flag".upper())
                        if (mindex != -1 and data.Data[mindex][i] == 0):
                            try:
                                sql = "INSERT INTO `%s` (`time`, `rt_last` ,`rt_last_vol`) VALUES (\'%s\',%f,%f)" % (
                                    var, data.Times[0], data.Data[0][i], data.Data[1][i])
                                self.cursor.execute(sql)
                            except:
    ...
    def mpretendwsq(self, codes, fields):
        pass

    def mwsq(self, codes, fields):
        try:
            if (codes.__len__() * fields.split(",").__len__() <= 100):
                return w.wsq(codes, fields)

            incre = int(100 / fields.split(",").__len__())
            data = w.wsq(codes[0:incre], fields)
            for var in range(incre, codes.__len__(), incre):
                tempdata = w.wsq(codes[var:(var + incre) if (var + incre <= codes.__len__()) else codes.__len__()],
                                 fields)
                ...
                if (tempdata.ErrorCode != 0):
                    ...
                    raise ConnectionError(u"网络错误:ErrorCode=%d" % tempdata.ErrorCode)
                if (tempdata.Times[0] - data.Times[0] > datetime.timedelta(seconds=15)):
                    QMessageBox.information(self, "!", u"网络延迟过大", QMessageBox.Yes)
                    raise ConnectionError(u"网络延迟过大")
            return data
        except:
            # QMessageBox.information(self, "!", u"未知网络错误", QMessageBox.Yes)
            while (1):
                print(u"未知网络错误")
                data = w.wsq(codes[:incre], fields)
                if (data.ErrorCode == 0):
                    return data

    def ok_bt_onclick(self):
        if (self.comboBox.currentIndex() == 0):
            codes = self.textEdit.toPlainText().__str__()
            self.codes = codes.split("\n")
            self.fields = "rt_last,rt_last_vol,rt_susp_flag"
            self.model.clear()
            self.model.setHorizontalHeaderLabels(self.fields.split(","))
            self.model.setVerticalHeaderLabels(self.codes)
            self.flag = 0
            self.t1.stop()
            self.t1.disconnect()
            self.t1.timeout.connect(self.mshow1)
            ...
            templist.extend(self.fields.split(","))
            templist.append("code")
            templist.extend(self.fields.split(","))
            self.model.clear()
            ...
            try:
                for var in codes:
                    self.codes.append(var.split(",")[1])
                    self.codes.append(var.split(",")[2])
                    self.codesName.append(var.split(",")[0])
                self.model.setVerticalHeaderLabels(self.codesName)
                for var in range(0, self.codes.__len__(), 2):
                    self.model.setItem(var / 2, 0, QStandardItem("%s" % self.codes[var]))
                    self.model.setItem(var / 2, self.fields.split(",").__len__() + 1,
                                       QStandardItem("%s" % self.codes[var + 1]))
            except:
...
app = QApplication([])
window = MainWindow("rt_last,rt_last_vol,rt_susp_flag,trade_status")
